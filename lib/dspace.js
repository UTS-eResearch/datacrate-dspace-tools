...
module.exports = function () {
    return {
    init: function(endpoint_url, metadata, out_dir, allStreams){
        this.allStreams = allStreams
        this.endpoint_url = endpoint_url;
        this.out_dir = out_dir ? out_dir : null;
    ...
    */
    get_bitstreams: function(bitstreams, item) {
        ...
        for (let stream of bitstreams){
            if (this.allStreams || stream.bundleName  === "ORIGINAL") {
                console.log(stream.bundleName )
                var new_file = {
                    "@id": `${stream.id}`,
                    "@type": "File",
                    "name": `${stream.name}`,
